---
layout: minimal
title: Publications
description: >
  Academic publications and preprints. *: co-first author.
hide_description: true
---

\* denotes co-first author.

## 2026

- **CryoHype: Reconstructing a thousand cryo-EM structures with transformer-based hypernetworks**<br>
  Jeffrey Gu\*, **Minkyu Jeon\***, Ambri Ma, Serena Yeung-Levy, Ellen D. Zhong
  **CVPR 2026**
  [Paper](https://arxiv.org/pdf/2512.06332) / [Project page](https://arxiv.org/pdf/2512.06332)

- **CHIMERA: Adaptive CacHe Injection and SeMantic Anchor Prompting for ZERo-shot ImAge Morphing with Morphing-oriented Metrics**<br>
  Dahyeon kye, Jaehun Sung, **Minkyu Jeon**, Jihyong Oh
  **ECCV 2026**
  [Paper](https://www.arxiv.org/abs/2512.07155) / [Project page](https://cmlab-korea.github.io/CHIMERA/)

- **ConforNets: Latents-Based Conformational Control in OpenFold3**<br>
  Minji Lee, Colin Kalicki, **Minkyu Jeon**, Aymen Qabel, Alisia Fadini, Mohammed AlQuraishi
  *arXiv preprint*, 2026
  [Paper](https://arxiv.org/abs/2604.18559)

---

## 2025

- **Separating Signal From Noise: A Self-Distillation Approach for Amortized Heterogeneous Cryo-EM Reconstruction**<br>
  **Minkyu Jeon\***, Jeffrey Gu\*, Ambri Ma, Serena Yeung-Levy, Vincent Sitzmann, Ellen D. Zhong
  **NeurIPS Workshop (MLSB) 2025 (Oral)**
  [Paper](https://openreview.net/pdf?id=YIvDrdPCo2) / [Project page](https://openreview.net/pdf?id=YIvDrdPCo2)

- **R3eVision: A Survey on Robust Rendering, Restoration, and Enhancement for 3D Low-Level Vision**<br>
  Weeyoung Kwon, Jeahun Sung, **Minkyu Jeon**, Chanho Eom, Jihyong Oh
  *arXiv preprint*, 2025
  [Paper](https://arxiv.org/abs/2506.16262)

---

## 2024

- **CryoBench: Diverse and challenging datasets for the heterogeneity problem in cryo-EM**<br>
  **Minkyu Jeon**, Rishwanth Raghu, Miro Astore, Geoffrey Woolard, Ryan Feathers, Alkin Kaz, Sonya M. Hanson, Pilar Cossio, Ellen D. Zhong
  **NeurIPS 2024 (Spotlight)**
  [Paper](https://arxiv.org/pdf/2408.05526) / [Project page](https://cryobench.cs.princeton.edu/)

---

## 2023

- **Randomly shuffled convolution for self-supervised representation learning**<br>
  Youngjin Oh\*, **Minkyu Jeon\***, Dohwan Ko, Hyunwoo J. Kim
  **Information Sciences Journal**, 2023
  [Paper](https://www.sciencedirect.com/science/article/pii/S0020025522013032) / [Code](https://github.com/minkyujeon/CROFFLE/tree/main/)

---

## 2022

- **SageMix: Saliency-Guided Mixup for Point Clouds**<br>
  Sanghyeok Lee\*, **Minkyu Jeon\***, Injae Kim, Yunyang Xiong, Hyunwoo J. Kim
  **NeurIPS 2022**
  [Paper](https://openreview.net/pdf?id=q-FRENiEP_d) / [Code](https://github.com/mlvlab/SageMix/)

- **k-SALSA: k-anonymous synthetic averaging of retinal images via local style alignment**<br>
  **Minkyu Jeon**, Hyeonjin Park, Hyunwoo J. Kim, Michael Morley, Hyunghoon Cho
  **ECCV 2022**
  [Paper](https://arxiv.org/pdf/2303.10824) / [Code](https://github.com/hcholab/k-salsa/)

---

## 2021

- **Learning to Balance Local Losses via Meta-Learning**<br>
  Seungdong Yoa, **Minkyu Jeon**, Youngjin Oh, Hyunwoo J. Kim
  **IEEE ACCESS**, 2021
  [Paper](https://ieeexplore.ieee.org/document/9541196)
